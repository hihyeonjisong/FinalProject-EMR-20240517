			...
			<div class="row justify-content-center align-items-center h-100">
				<div class="col-xxl-6 col-xl-7 col-lg-7 col-md-7 col-sm-8 col-12">
					<div class="p-5">
                        ...
                        <p class="h5 fw-semibold mb-2">아이디 찾기</p>
						<br>
						<form action="/signup.do" method="post" id="signupForm">
							...
							<div id="isFoundedId" style = "padding-top : 10px; padding-bottom : 10px; text-decoration: underline;">
								
							</div>
							<div class="col-xl-12 d-grid mt-2">
								<input type="button" class="btn btn-lg btn-primary1" id="findIdBtn" value="아이디 찾기"
								style="background-color: #afe1f6; color: white;">
							</div>
<%-- 							<sec:csrfInput /> --%>
						</form>
						<br>
						<p class="h5 fw-semibold mb-2">비밀번호 찾기</p>
						<br>
						<form action="/public/signup.do" method="post" id="signupForm">
							<div class="row gy-3">
								<div class="col-xl-12 mt-0">
									<label for="signup-firstname" class="form-label text-default">이름</label>
									<input type="text" class="form-control form-control-lg" name="memName2" id="memName2" placeholder="이름">
								...
								<div class="col-xl-12 d-grid mt-2">
									<input type="button" class="btn btn-lg btn-primary1" id="findPwBtn"
										value="비밀번호 찾기" style="background-color: #afe1f6; color: white;">
								</div>
								<div id="isFoundedCount" style = "padding-top : 10px; padding-bottom : 10px; text-decoration: underline;">
		...
		$(function() {
			...
			$(document).ready(function() {
			    $('#findIdBtn').click(function() {
			        var memName = $('#memName').val(); // memId 입력 값 가져오기
			        var memTel = $('#memTel').val(); // memRegno 입력 값 가져오기
			        
			        // AJAX 요청
			        $.ajax({
			            type: 'POST',
			            url: '/patient/findId', // 비밀번호 찾기를 처리하는 서버의 URL
			            data: {
			                memName: memName,
			                memTel: memTel
			            },
			            success: function(response) {
			                // 서버로부터의 응답을 처리하는 코드
			                // 여기에서 memPw 정보를 처리하거나 표시할 수 있습니다.
			                console.log(response); // memPw 값을 콘솔에 출력하는 예시
			                let isFoundedId = $('#isFoundedId');
							if (response == null || response == "") {
				                isFoundedId.text('회원님의 ID가 존재하지 않습니다.');
							} else {
				                isFoundedId.text('회원님의 ID는 "' + response + '" 입니다.');
							}
			            },
			            error: function(xhr, status, error) {
			                // AJAX 요청이 실패했을 때 처리하는 코드
			                console.error(error); // 에러 메시지를 콘솔에 출력하는 예시
			            }
			        });
			    });
			});
			...
			$(document).ready(function(){
				$('#findPwBtn').click(function(){
					var memName = $('#memName2').val(); // memName 입력 값 가져오기
					console.log(memName);
					var memId = $('#memId').val();	   // memId 입력 값 가져오기
					var memEmail = $('#memEmail').val(); // memEmail 입력 값 가져오기
					
				$.ajax({
					type : 'POST',
					url : '/patient/checkCount',
					data: {
						memName : memName,
						memId : memId,
						memEmail : memEmail
					...
					let isFoundedCount = $('#isFoundedCount');
					if (response == null || response == ""){
						isFoundedCount.text('회원님의 정보가 존재하지 않습니다.');
					}else {
						isFoundedCount.text('임시비밀번호가 이메일에 전송되었습니다.')
						$.ajax({
							type : 'POST',
							url : '/patient/changePw',
							data : {
								memName : memName,
								memId : memId,
								memEmail : memEmail
